...
- Agent 工具意图：覆盖搜索调研、下载论文、知识问答、粗读论文、精读论文、生成 Wiki、生成简报等任务。
...
- AI 生成画像：根据快捷画像和历史回答生成可读的关注偏好、推荐方向、阅读目标和追问。
- 推荐队列：展示论文推荐理由、画像匹配、方法价值、实验可信度、可复现性、新颖性、近期趋势等评分因素。
- 推荐反馈：可对推荐打星，反馈会更新推荐模型和后续排序。
- 研究雷达：按精读候选、速读候选、跳过原因展示每日候选论文，显示 BM25、embedding、画像分数和 LLM refine 状态。

### 论文收集与多源抓取

- arXiv 搜索入库：支持 query、最大数量、排序和时间范围。
- 多源搜索：支持 arXiv、OpenReview、Semantic Scholar、OpenAlex、DBLP、bioRxiv。
- 渠道建议：根据查询词推荐适合的论文源。
- 主题订阅：支持新建、编辑、启停、删除主题，配置关键词、描述、抓取配额、自动精读数量、抓取时间和渠道。
- 关键词建议：根据主题描述生成关键词建议。
- 手动抓取：可立即抓取某个主题，并轮询抓取状态。
- 学科订阅：支持查看 CS 分类，创建分类订阅，设置 daily limit，启停、删除和手动抓取。
...
- Wiki：支持单篇论文 Wiki、主题 Wiki、异步生成、历史记录、详情查看和删除。
- 研究简报：支持手动生成、任务进度、历史简报、详情查看和删除。
- 设置：支持全局 AI 后端、LLM Provider、模型、API Key、Codex CLI、邮件 SMTP、日报开关、Cron、收件人、自动精读和健康检查配置。
...
packages/ai/              推荐、雷达、解析、RAG、Agent、简报等核心服务
packages/integrations/    论文源和外部服务集成
packages/storage/         数据模型和数据库访问
tests/                    后端核心功能测试
docs/                     当前文档、答辩材料和分工说明
data/                     本地数据库、PDF、简报输出，默认不提交
```
...
- `POST /topics/{topic_id}/fetch`：主题订阅抓取。
- `GET /cs/feeds`：计算机学科订阅。
...
- A：用户画像、个性化推荐、研究雷达。
- B：论文收集、主题订阅、多源渠道、OpenReview、计算机学科订阅。
- C：论文库、论文详情、PDF 阅读助手、粗读、精读、推理链、RAG、标签。
- D：Agent、Wiki、研究简报、设置、运维、认证、全局任务。
