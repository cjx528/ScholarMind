...
支持手动配置研究方向、关键词、偏好和反馈，也支持通过 LLM 生成画像。LLM 配置属于全局设置，不再只绑定用户画像页面。

### 2. 研究雷达

- 页面：`/radar`
- 后端：`/recommendation/daily-radar`
- 服务：`packages/ai/daily_radar_service.py`

按精读候选、速读候选和跳过原因组织论文，目标是让用户每天打开后能直接判断阅读优先级。

### 3. 论文收集与多源渠道

- 页面：`/collect`
- 主题接口：`apps/api/routers/topics.py`
- 学科订阅接口：`apps/api/routers/cs_feeds.py`
- 集成：`packages/integrations/`

当前支持 arXiv、OpenReview、Semantic Scholar、OpenAlex、DBLP、bioRxiv 和计算机学科订阅。主题订阅支持关键词建议、启停状态、渠道选择和手动抓取。
...
粗读用于快速判断论文价值，精读用于方法、实验和风险分析，推理链用于梳理论文论证结构。三者都可以作为论文助手的追问上下文。
...
- 每日简报：`frontend/src/pages/DailyBrief.tsx`、`packages/ai/brief_service.py`

用于把单篇阅读扩展到长期知识库和每日研究总结。
