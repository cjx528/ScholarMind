...
- 用户画像页面能正常加载已有 profile，只展示画像配置、生成和保存，不展示推荐队列。
...
- AI 生成画像能输出自然中文的关注偏好、研究方向、阅读目标和追问，不应暴露提示词、JSON 或后端报错文本。
- 论文库、Agent、搜索排序和论文精读个性化分析能在各自流程中参考用户画像。
...
- 侧边栏工具入口都能跳转：用户画像、论文收集、论文库、Wiki、看板、主题统计。
...
- 看板能展示系统状态、成本分析、Pipeline 运行记录和最近活动。
