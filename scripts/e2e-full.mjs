...
(async () => {
  ...
  logo.includes("ScholarMind") ? pass("Logo 存在") : fail("Logo 缺失");
  ...
  if (chatInput) {
    ...
    if (sendBtn) {
      ...
      const hasReply = chatBody.includes("ScholarMind") || chatBody.includes("论文") || chatBody.length > 500;
  ...
  if (paperLinks.length > 0) {
    ...
    // 检查 PDF 阅读助手入口
    detailBody.includes("阅读原文")
      ? pass("PDF 阅读助手入口") : info("PDF 阅读助手入口未显示（可能无 PDF）");
  ...
  // 验证成本中文标签
  for (const label of ["粗读分析", "推理链分析", "PDF 阅读助手", "RAG 问答"]) {
